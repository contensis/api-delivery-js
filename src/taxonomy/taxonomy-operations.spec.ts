...
describe('Taxonomy Operations', function () {
	...
	it('Resolve Children with Key by Options', () => {
		...
		client.taxonomy.resolveChildren({ node: '0/1', order: 'alphabetical', childDepth: 99, language: 'fr-FR' });
		expect(global.fetch).toHaveBeenCalled();
	...
	it('Resolve Children with Key by Options - Ignore order by defined', () => {
		...
		client.taxonomy.resolveChildren({ node: '0/1', order: 'defined', childDepth: 99, language: 'fr-FR' });
		expect(global.fetch).toHaveBeenCalled();
